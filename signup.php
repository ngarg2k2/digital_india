<?php 
    if(isset($_POST['username']))
    {
        ...
        {
          $sqltest = "SELECT `username` FROM `pharmacy_se`.`users` WHERE username='$username'";
          if($con->query($sqltest)==true)
          {
            ...
            {
              $sql = "INSERT INTO `pharmacy_se`.`users` (`username`, `password`, `firstname`, `lastname`, `pcode`, `phone`, `email`, `house`, `area`, `city`, `pincode`, `stt`,`req`) VALUES 
              ('$username', '$password', '$firstname', '$lastname', '$pcode', '$phone', '$email', '$house', '$area', '$city', '$pincode', '$state', '$req');";
