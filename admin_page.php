<?php
        if(isset($_POST['med_id']))
        {
            ...
            $sqlcheck1 = "SELECT `med_id` FROM  `pharmacy_se`.`medicines` WHERE med_id=$med_id;";
            ...
            if (strlen(strval($med_id))<8)
            {
                if($con->query($sqlcheck1)==true)
                {
                    ...
                    {
                        $sqlcheck5 = "SELECT `med_name` FROM `pharmacy_se`.`medicines` WHERE med_id=$med_id;";
                        $sqlcheck2 = "SELECT `mg` FROM `pharmacy_se`.`medicines` WHERE med_id=$med_id;";
                        ...
                        {
                            $sqlcheck3 = "SELECT `available` FROM `pharmacy_se`.`medicines` WHERE med_id=$med_id;";
                            $resultcheck3 = $con->query($sqlcheck3);
                            $row3 = $resultcheck3->fetch_assoc();
                            $available = $row3['available'];
                            $sqlupdate = "UPDATE `pharmacy_se`.`medicines`
                        ...
                        else
                        {
                            ...
                            while ($k==0)
                            {
                                $sqlcheck4 = "SELECT * FROM `pharmacy_se`.`medicines` WHERE med_id=$index;";
                            ...
                            try
                            {$sqlnew = "INSERT INTO `pharmacy_se`.`medicines` (`med_id`, `company`, `med_name`, `mg`, `available`, `price`) 
                            VALUES ('$index', '$company', '$medicine_name', '$mg', '$amount', '$price');";
                    ...
                    {
                        try{
                              $sql = "INSERT INTO `pharmacy_se`.`medicines` (`med_id`, `company`, `med_name`, `mg`, `available`, `price`) 
                        VALUES ('$med_id', '$company', '$medicine_name', '$mg', '$amount', '$price');";
